/*
	...
	Une piste pourrait être de prendre la répétition de lettre la plus à droite, en gros la dernière. Ou de faire du multi-threading car le programme est très linéaire.
...
void method2(const char* chaine, rsize_t longueur);
...
int main() {
    ...
    Soit 6,66 fois plus rapide pour la méthode 3
    Contre 2,99 fois plus pour la méthode 2
...
void method2(const char* chaine, size_t longueur) {
    ...
    Puisque une lettre est un indice de 0 à 25, on peut voir si elle est répétée ou non en utilisant un masque de bits (ici j'ai préféré faire une soustraction, c'est plus simple) (unsigned int = 32 bits)
	Je vais ainsi pouvoir les stockés dans un variable unsigned int qui fera office de tableau de booléens (0 = lettre non présente, 1 = lettre présente)
	un char est 8 bits 0000 0000
    */
    ...
    for (unsigned int i = 0; i < longueur; ++i) {
        ...
        for (unsigned int j = i; j < SOUS_CHAINE_LEN + i; ++j) {
            ...
            */
            ...
            La comparaison avec un AND nous renvoie si la lettre est déjà dans la chaîne
            ...
            */
			...
			// Une fois la sous-chaîne pleine
...
void method3(const char* chaine, size_t longueur) {
    ...
    while (i < longueur) {
        ...
        // Si la lettre n'est pas dans la sous-chaîne
        if (!(sous_chaine & (1 << (chaine[i] - 'a')))) {
            ...
            while (chaine[index] != chaine[i]) {
                // Enlève la lettre la plus à gauche de la sous-chaîne
